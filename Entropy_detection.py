...
#filewriter.writerow(['ID', 'Entropy','Packet','byteCount','flowCount','flowDuration','ddos'])
      
      
      
      def createDataset2(bytecnt,flowcnt,flowduration) :
...
class l3_switch (EventMixin):
  ...
  def createDataset1(self, ent) :
        print('writing data')
        with open('networkSimulation.csv', 'a') as csvfile:
          filewriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
         # filewriter.writerow([ID, Entropy','Packet','byteCount','flowCount','flowDuration','ddos'])
          filewriter.writerow([self.idx, ent])
        self.idx+=1
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if isinstance(packet.next, ipv4):
      ...
      #self.createDataset1(str(ent_obj.value))
...
def createDataset2(switchMac,bytecnt,flowcnt,flowduration,packetCount) :
        with open('networkSimulation.csv', 'a') as csvfile:
          filewriter = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
          filewriter.writerow([switchMac,bytecnt, flowcnt, flowduration, packetCount, 3])
...
def _handle_flowstats_received (event):
    ...
    for flow in event.stats:
        ...
        flowlist = {"flow_Duration": flow.duration_sec, "byte_count": flow.byte_count, "packet_count": flow.packet_count}
        #print("FlowList : ",flowlist,"\n")
        #Increment the counters
        flowDurationSum += flow.duration_sec
    ...
    print("-"*50,p_count,b_count,flow_count)
    createDataset2(dpidToStr(event.connection.dpid),b_count,flow_count,flowDurationSum,p_count)

    # call svm model and pass data -- kaggle
    # based on output label call os.exit  
...
def launch (fakeways="", arp_for_unknowns=None, wide=False):
  akeways = fakeways.replace(","," ").split()
  ...
  if flow_list > fth and start<end:
